    ...
    class Program
    {
        static async Task Main()
        {
            ...
            if (File.Exists(filePath))
            {
                ...
                var items = ArrayPool<CsvEmailBeaconLogRecord>.Shared.Rent(100); // make the record a struct?
                ...
                {
                    ArrayPool<CsvEmailBeaconLogRecord>.Shared.Return(items, clearArray: true);
                }
            }
        }

        private static void ParseLines(CsvEmailBeaconLogRecord[] itemsArray, ref ReadOnlySequence<byte> buffer, ref int position)
        {
        ...
        private static CsvEmailBeaconLogRecord ParseLine(in ReadOnlySequence<byte> line)
        {
        ...
        internal static class LineParser
        {
            ...
            public static CsvEmailBeaconLogRecord ParseLine(ReadOnlySpan<byte> line)
            {
                if (line[0] == Hash) return null;

                var tabCount = 1;

                var record = new CsvEmailBeaconLogRecord();
